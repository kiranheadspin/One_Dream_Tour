...
export async function getCaptainTeamOrNull(profileId: string): Promise<Team | null> {
  ...
  return {
    id: data.id, name: data.member_name ?? data.name, officialName: data.name, company: company?.legal_name ?? "", city: city?.city as Team["city"], captainName: profile?.full_name ?? "",
    captainEmail: "", registrationStatus: data.registration_status as Team["registrationStatus"], rulesAcceptedAt: data.rules_accepted_at ?? undefined, rulesVersion: data.rules_version ?? undefined,
