...
export interface AppSession { userId: string; name: string; email: string; role: AppRole; demo: boolean; }
...
export async function getSession(): Promise<AppSession | null> {
  if (isDemoMode) {
    ...
    return role === "admin"
      ? { userId: "demo-admin", name: "Operations Admin", email: "admin@demo.local", role, demo: true }
      : { userId: "demo-captain", name: "Meera Nair", email: "meera@example.com", role, demo: true };
  }
  ...
  if (!role) return null;
  return { userId: user.id, name: profile?.full_name ?? user.user_metadata?.full_name ?? user.email ?? "User", email: user.email ?? "", role, demo: false };
}
