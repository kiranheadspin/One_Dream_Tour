          ...
          <tbody>
            {leads.map((lead) => (
              <tr key={lead.id} className="border-t hover:bg-slate-50/70">
                <td className="px-5 py-4"><div className="flex items-center gap-2"><Link href={`/admin/leads/${lead.id}`} className="font-semibold text-[#081326] hover:underline">{lead.captainName}</Link>{lead.isNew ? <span className="rounded-full bg-sky-100 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-sky-800">New</span> : null}</div><p className="mt-1 font-mono text-[11px] text-slate-400">{lead.reference}</p></td>
      ...
      <div className="grid md:hidden">
        {leads.map((lead) => (
          <article key={lead.id} className="border-b p-4 last:border-0 sm:p-5">
            <div className="flex justify-between gap-4"><div className="min-w-0"><div className="flex items-center gap-2"><Link href={`/admin/leads/${lead.id}`} className="font-semibold text-[#081326]">{lead.captainName}</Link>{lead.isNew ? <span className="rounded-full bg-sky-100 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-sky-800">New</span> : null}</div><p className="mt-1 truncate text-xs text-slate-500">{lead.company} · {lead.city}</p><p className="mt-1 break-all text-xs text-slate-500">{lead.email}</p></div><Link href={`/admin/leads/${lead.id}`} aria-label={`Open ${lead.captainName}`} className="grid size-10 shrink-0 place-items-center rounded-lg bg-[#313999]/8"><ArrowUpRight aria-hidden="true" className="size-4 text-[#313999]" /></Link></div>
