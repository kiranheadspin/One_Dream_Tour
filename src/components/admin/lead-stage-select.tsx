...
import { LEAD_STAGES, LEAD_STAGE_SELECT_CLASS, type LeadStage } from "@/lib/constants";
...
export function LeadStageSelect({ id, stage }: { id: string; stage: LeadStage }) {
  ...
  return <NativeSelect size="sm" disabled={saving} value={value} onChange={(event)=>change(event.target.value as LeadStage)} className={`w-[160px] ${LEAD_STAGE_SELECT_CLASS[value]}`}>{LEAD_STAGES.map(item=><NativeSelectOption key={item} value={item}>{item}</NativeSelectOption>)}</NativeSelect>;
}
